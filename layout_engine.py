    ...
    for thickness, group in grouped:
        for strategy in ['row', 'column']:
            ...
            while not remaining_parts.empty:
                ...
                layout_filename = os.path.join(output_folder, f"layout_thickness_{thickness}_sheet_{sheet_num}_{strategy}.png")
                plt.savefig(layout_filename, bbox_inches='tight', dpi=300)
                plt.close()

                remaining_parts = remaining_parts.drop(placed_indices)
                sheet_num += 1

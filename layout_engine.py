...
def generate_layout_and_summary(csv_path, output_folder, sheet_width=48.0, sheet_height=96.0):
    ...
    for thickness, group in grouped:
        for strategy in ['row', 'column']:
            sheet_number = 1
            remaining_parts = group.copy()

            while not remaining_parts.empty:
                fig, ax = plt.subplots(figsize=(8.27, 11.69))  # A4 portrait in inches
                ax.set_xlim(0, sheet_width)
                ax.set_ylim(0, sheet_height)
                ax.invert_yaxis()
                ax.set_aspect('equal')
                ax.set_title(f"Thickness {thickness} - Sheet {sheet_number} ({strategy})")
                ...
                for idx, part in remaining_parts.iterrows():
                ...
                layout_filename = os.path.join(output_folder, f"layout_thickness_{thickness}_{strategy}_sheet{sheet_number}.png")
                plt.savefig(layout_filename, bbox_inches='tight', dpi=300)
                plt.close()

                remaining_parts = remaining_parts.drop(index=placed_indices)
                sheet_number += 1
    ...
    for thickness, group in grouped:
        ...
        for _, row in group.iterrows():
            sheet.append([row.get("Part Name", ""), row["Width"], row["Height"], row["Thickness"], row.get("Material", ""), row["Quantity"]])

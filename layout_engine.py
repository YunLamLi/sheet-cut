    ...
    def generate_layout_and_summary(csv_path, output_folder, sheet_width=48.0, sheet_height=96.0):
        ...
        for thickness, group in grouped:
            for strategy in ['row', 'column']:
                ...
                while not all(placed):
                    ...
                    ax.invert_yaxis()
                    ax.set_aspect('equal')
                    ax.set_title(f"Thickness {thickness} - Sheet {sheet_num} ({strategy})")
                    ...
                    for i, part in enumerate(parts):
                        ...
                        if strategy == 'row':
                            ...
                            if y_cursor + ph > sheet_height:
                                continue
                            ax.add_patch(patches.Rectangle((x_cursor, y_cursor), pw, ph, linewidth=1, edgecolor='black', facecolor='lightgrey'))
                            label = f"{part.get('Part Name', 'Unnamed')}\n{pw:.2f}x{ph:.2f}"
                    ...
                    if something_placed:
                        filename = os.path.join(output_folder, f"layout_thickness_{thickness}_{strategy}_sheet_{sheet_num}.png")
        ...
        for thickness, group in grouped:
            ...
            for _, row in group.iterrows():
                sheet.append([row.get("Part Name", "Unnamed"), row["Width"], row["Height"], row["Thickness"], row.get("Material", ""), row["Quantity"]])
